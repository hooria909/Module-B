...
function brandSelection() {
   let brand = document.getElementById("select-brand").value;
    let models = document.getElementById("select-model");
    ...
    placeholderOption.id = "placeHolder2";
    models.appendChild(placeholderOption);

    let modelKeys = Object.keys(mobiles[brand]);
    
    for (let i = 0; i < modelKeys.length; i++) {
      let opt = document.createElement("option");
      opt.textContent = modelKeys[i];
...
function modelData() {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (typeof v !== "object") {
      ...
      div.innerHTML += k + ": " + Object.keys(v) + "<br>";
    }
  }
